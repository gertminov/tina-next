...
function Header() {
    ...
    return (
        <nav className="fixed z-10 flex h-[10vh] w-full flex-none items-center justify-between border-b border-black bg-backgroundB px-4">
            <div className="flex">
                <Link
                    href={"/"}
                    className="h-full w-32 font-serif text-5xl"
                    aria-label="Valentina Werr Logo / Home button"
                >
                    <svg
                        fill="#000000"
                        id="Ebene_1"
            ...
            >
                <div className="space-y-2 z-40">
                    <div className={`${menuLine} ${!showMenu ? "opacity-100": "rotate-45 translate-y-3"}`}></div>
                    <div className={`${menuLine} ${!showMenu ? "opacity-100": "opacity-0"}`}></div>
                    <div className={`${menuLine} ${!showMenu ? "opacity-100" : "-rotate-45 -translate-y-3"}`}></div>
                </div>
            </button>
            {showMenu && (
